...
public class Kidsbus extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sName.setText(" "+name+ " 어린이 탑승 여부 달력!");
...
//----------------------------------------------------------
        //, on(off)에서 off(on)로 바뀔때 이벤트 처리함.
        for(int i=0; i<tb.length; i++) {
            check=tb[i].getText().toString();
            tb[i].setOnClickListener(new Button.OnClickListener() {
                @Override
                public void onClick(View view) {
                    {
                        if (check.equals("탑승O")) { // off -> on
                            int position = (Integer) view.getTag();
                            System.out.println("Clikced : " + position);
                            Toast.makeText(getApplicationContext(), position+ "/ year :" + c[position].get(Calendar.YEAR)
                                    + "\nmonth :" + (c[position].get(Calendar.MONTH) + 1) + "\nday :"
                                    + c[position].get(Calendar.DATE), Toast.LENGTH_SHORT).show();
                            tb[position].setText("탑승X");
                        } else {  // on -> off
                            int position = (Integer) view.getTag();
                            System.out.println(" unClikced : " + position);
                            Toast.makeText(getApplicationContext(), "year :" + c[position].get(Calendar.YEAR)
                                    + "month :" + (c[position].get(Calendar.MONTH) + 1) + "day :"
                                    + c[position].get(Calendar.DATE), Toast.LENGTH_SHORT).show();
                            tb[position].setText("탑승O");
                        }
                    }
                }
            });
